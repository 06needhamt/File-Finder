...
namespace Files
{
    class Program
    {
        static string sourcedir = @"C:\Users\Tom\Desktop\CodingStuff\p95v285.source"; // variable to hold the directory to be searched for files
        static string destinationdir = @"C:\Users\Tom\Desktop\filesp95"; // variable to hold the directory to save the found files to
        static string filetypes = "*.asm"; // variable to hold the types of files to look for
        ...
        {
            return !(System.IO.Directory.EnumerateFileSystemEntries(path).Any()); // Returns TRUE if there are no files in the directory, returns FALSE if there are files in the directory
        }
